...
- **`ui.py`**: Управляет окном PySide6 и взаимодействием с пользователем. Поддерживает динамическое управление видимостью деталей через дерево проекта (Scene Graph).
...
### 4. Профильное моделирование (Profile-First Design)
Для сложных узлов сопряжения (например, козырьки, обхватывающие зажимы) рекомендуется использовать экструзию 2D-профилей с перекрытием (overlap) вместо сложения простых примитивов. Это обеспечивает монолитность и прочность деталей.
